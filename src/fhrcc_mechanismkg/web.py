...
.recenter { position:absolute; top:10px; right:10px; z-index:2; font-size:13px; }
...
<main>
  <div class="cywrap"><div id="cy" role="img" aria-label="Interactive graph of FH-loss mechanisms. A table of all edges follows below."></div><button type="button" id="recenter" class="recenter" title="Fit the whole graph in view">Recenter</button></div>
...
(function () {
  ...
  function styles() {
    ...
    const s = [
      ...
      { selector: 'edge.nbr', style: { 'opacity': 1, 'z-index': 25, 'line-color': col('molecular'), 'target-arrow-color': col('molecular') } },
    ...
    for (const g of Object.keys(D.groupLabels)) s.push({ selector: 
ode[group = "${g}"]`, style: { 'background-color': col(g), 'border-color': col(g) } });
    return s;
  }
  ...
  $('recenter').onclick = () => { if (cy) cy.fit(undefined, 30); };
